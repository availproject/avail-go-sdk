...
func NewTxLogger(txHash prim.H256, enabled bool) TxLogger {
...
func (this *TxLogger) Enabled(value bool) {
	this.enabled = value
}

func (this *TxLogger) LogWatcherRun(waitFor uint8, blockHeightTimeout uint32) {
...
func (this *TxLogger) LogWatcherNewBlock(block *RPCBlock, blockHash prim.H256) {
	if !logrus.IsLevelEnabled(logrus.DebugLevel) || !this.enabled {
		return
	}
...
func (this *TxLogger) LogWatcherTxFound(details *TransactionDetails) {
	if !logrus.IsLevelEnabled(logrus.DebugLevel) || !this.enabled {
		return
	}
...
func (this *TxLogger) LogWatcherStop() {
	if !logrus.IsLevelEnabled(logrus.DebugLevel) || !this.enabled {
		return
	}
...
func (this *TxLogger) LogTxSubmitted(keypair *subkey.KeyPair, mortality uint64) {
	if !logrus.IsLevelEnabled(logrus.DebugLevel) || !this.enabled {
		return
	}
...
func (this *TxLogger) LogTxSubmitting(keypair *subkey.KeyPair, payload *metadata.Payload, nonce uint32, appId uint32) {
	if !logrus.IsLevelEnabled(logrus.DebugLevel) || !this.enabled {
		return
	}
